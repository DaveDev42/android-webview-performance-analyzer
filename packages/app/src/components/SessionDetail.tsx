...
import type { Session, StoredMetric, StoredNetworkRequest, PerformanceMetrics } from "../types";
...
export function SessionDetail({ session, onClose, onDelete, onExport }: SessionDetailProps) {
  ...
  const loadSessionData = async () => {
    ...
    try {
      // Load metrics
      const storedMetrics = await invoke<StoredMetric[]>("get_session_metrics", {
        sessionId: session.id,
        metricType: "performance",
      });

      // Parse metrics data
      const parsedMetrics: PerformanceMetrics[] = storedMetrics.map((m) => {
      ...
      const requests = await invoke<StoredNetworkRequest[]>("get_session_network_requests", {
        sessionId: session.id,
      });
  ...
  const avgHeapUsed = metrics.length > 0
  ...
  const avgDomNodes = metrics.length > 0
    ? metrics.reduce((sum, m) => sum + (m.dom_nodes || 0), 0) / metrics.length
    : 0;
  const totalRequests = networkRequests.length;
  const failedRequests = networkRequests.filter((r) => r.status_code && r.status_code >= 400).length;
                    ...
                    <tbody>
                      {networkRequests.map((req) => (
                        <tr key={req.id} className="border-t border-gray-700">
                          ...
                          <td className="p-3 text-gray-300 truncate max-w-md">
                            {req.url}
                          </td>
